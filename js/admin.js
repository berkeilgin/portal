...
function toggleToolFlag(id, field, btn) {
  ...
  if (tool) {
    ...
    btn.classList.toggle('on', tool[field]);
  }
}
...
function openToolModal(id = null) {
  ...
  if (id) {
    ...
    document.getElementById('toolCat').value = t.cat;
    document.getElementById('toolIcon').value = t.icon || 'logo.png';
...
function handleFiles(files) {
  for (let f of files) {
    ...
    reader.onload = e => {
      uploadQueue.push({ name: f.name, b64: e.target.result.split(',')[1], type: f.type });
      renderUploadPreview();
    };
...
function renderUploadPreview() {
  ...
  container.innerHTML = uploadQueue.map((f, i) => `
    <div class="upload-thumb">
      ...
      <span>${f.name}</span>
      <button class="btn btn-ghost btn-sm" onclick="removeUpload(${i})">✕</button>
    </div>
  `).join('');
...
async function uploadFiles() {
  ...
  for (const f of uploadQueue) {
    try {
      let sha = null;
      try { const exist = await ghGet(`logos/${f.name}`); sha = exist.sha; } catch(e) {}
      await ghPut(`logos/${f.name}`, f.b64, sha, `Yeni ikon: ${f.name}`);
      statusDiv.innerHTML += `<p style="color:var(--accent)">✓ ${f.name} yüklendi</p>`;
    } catch(e) {
      statusDiv.innerHTML += `<p style="color:var(--accent3)">✕ ${f.name}: ${e.message}</p>`;
    }
  }
  uploadQueue = [];
  renderUploadPreview();
  btn.disabled = false;
  btn.innerHTML = '⬆️ GitHub\'a Yükle';
...
async function saveToGitHub() {
  ...
  try {
    const res = await ghPut('tools.json', b64Encode(JSON.stringify(data, null, 2)), fileSha, `Admin güncelleme ${new Date().toLocaleString('tr')}`);
    fileSha = res.content.sha;
    setStatus('ok', 'Kaydedildi!');
    toast('✅ GitHub\'a kaydedildi. 1-2 dk içinde yayınlanır.');
  } catch(e) {
    setStatus('err', 'Hata: ' + e.message);
    toast('Kayıt başarısız: ' + e.message, 'error');
  } finally {
    btn.disabled = false;
    btn.innerHTML = '💾 Kaydet & Yayınla';
  }
}
...
function setStatus(type, msg) {
  ...
  el.textContent = msg;
  dot.style.background = type === 'ok' ? 'var(--accent2)' : type === 'err' ? 'var(--accent3)' : 'var(--accent4)';
}
