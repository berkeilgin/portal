...
let checkedMonitoringIdsStep3 = new Set();
let currentReportView = 'proje';
let cachedReportRows = []; // Filtreleme için saklanan ham satırlar
let cachedReportHeaders = [];
...
const filterProje = document.getElementById('filterProjeStep3');
const filterDeğerlendirici = document.getElementById('filterDeğerlendiriciStep3');
const filterGrup = document.getElementById('filterGrupStep3');
const filterRow = document.getElementById('reportFilterRowStep3');
...
function loadMainExcelS3(file) {
  ...
  reader.onload = e => {
    try {
      ...
      // Otomatik rapor oluşturma KALDIRILDI - sadece buton ile oluşacak
    } catch (err) {
...
function applyFiltersToReport() {
  const projeFilter = filterProje ? filterProje.value.toLowerCase().trim() : '';
  const degerFilter = filterDeğerlendirici ? filterDeğerlendirici.value.toLowerCase().trim() : '';
  const grupFilter = filterGrup ? filterGrup.value.toLowerCase().trim() : '';

  if (!cachedReportRows.length) {
    reportBodyS3.innerHTML = `<tr><td colspan="${cachedReportHeaders.length}">Veri yok</td></tr>`;
    return;
  }

  const filtered = cachedReportRows.filter(row => {
    // row bir dizi, indekslere göre filtrele
    let proje = '', deger = '', grup = '';
    ...
    if (projeFilter && !proje.toLowerCase().includes(projeFilter)) return false;
    if (degerFilter && !deger.toLowerCase().includes(degerFilter)) return false;
    if (grupFilter && !grup.toLowerCase().includes(grupFilter)) return false;
    return true;
  });

  if (filtered.length) {
    reportBodyS3.innerHTML = filtered.map(row => `<tr>${row.map(cell => `<td>${cell}</td>`).join('')}</tr>`).join('');
  } else {
    reportBodyS3.innerHTML = `<tr><td colspan="${cachedReportHeaders.length}">Filtreye uygun veri yok</td></tr>`;
  }
}

// ---------- Rapor Oluştur ----------
function generateReportS3() {
  console.log('📊 generateReportS3 çalıştı, reportMainData uzunluğu:', reportMainData.length);
  ...
  if (!Object.values(reportHistory3).some(arr => arr.length)) {
    alert('Dağıtım geçmişi (JSON) yüklenmemiş veya boş.');
    return;
  }
  if (!checkedMonitoringIdsStep3.size) {
    alert('Kontrol Edilenler (Monitoring ID) listesi yüklenmemiş veya boş.');
    return;
  }
  ...
  if (currentReportView === 'proje') {
    ...
    headers = ['Proje Adı', 'Dağıtılan', 'Giriş Yapılan', 'Giriş Yapılmayan', 'Giriş Oranı'];
    rows = Array.from(map.entries()).map(([p, s]) => {
      ...
      return [p, s.dist, s.checked, girisYapilmayan, oran];
    });
    // Sıralama: Proje A-Z
    ...
    rows = Array.from(map.values()).map(v => {
      ...
      return [v.proje, v.kisi, v.dist, v.checked, girisYapilmayan, oran];
    });
    ...
    rows = Array.from(map.entries()).map(([k, s]) => {
      const girisYapilmayan = s.dist - s.checked;
      const oran = s.dist > 0 ? ((s.checked / s.dist) * 100).toFixed(1) + '%' : '0%';
      return [k, s.dist, s.checked, girisYapilmayan, oran];
    });
    rows.sort((a,b) => a[0].localeCompare(b[0]));
  } else if (currentReportView === 'group') {
    const map = new Map();
    ...
    rows = Array.from(map.entries()).map(([g, s]) => {
      const girisYapilmayan = s.dist - s.checked;
      const oran = s.dist > 0 ? ((s.checked / s.dist) * 100).toFixed(1) + '%' : '0%';
      return [g, s.dist, s.checked, girisYapilmayan, oran];
    });
    rows.sort((a,b) => a[0].localeCompare(b[0]));
  }

  // Önbelleğe al (filtreleme için)
  cachedReportHeaders = headers;
...
function exportReportS3() {
  ...
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(Array.from(projMap.entries()).map(([p, s]) => ({ 'Proje': p, 'Dağıtılan': s.dist, 'Giriş Yapılan': s.checked, 'Giriş Yapılmayan': s.dist - s.checked, 'Giriş Oranı': s.dist > 0 ? ((s.checked / s.dist) * 100).toFixed(1) + '%' : '0%' }))), 'Proje_Bazlı');
  ...
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(Array.from(pkMap.values()).map(v => ({ 'Proje': v.proje, 'Değerlendirici': v.kisi, 'Dağıtılan': v.dist, 'Giriş Yapılan': v.checked, 'Giriş Yapılmayan': v.dist - v.checked, 'Giriş Oranı': v.dist > 0 ? ((v.checked / v.dist) * 100).toFixed(1) + '%' : '0%' }))), 'Proje_Kisi');
  ...
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(Array.from(kisiMap.entries()).map(([k, s]) => ({ 'Değerlendirici': k, 'Dağıtılan': s.dist, 'Giriş Yapılan': s.checked, 'Giriş Yapılmayan': s.dist - s.checked, 'Giriş Oranı': s.dist > 0 ? ((s.checked / s.dist) * 100).toFixed(1) + '%' : '0%' }))), 'Kisi_Bazlı');
  ...
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(Array.from(groupMap.entries()).map(([g, s]) => ({ 'Grup': g, 'Dağıtılan': s.dist, 'Giriş Yapılan': s.checked, 'Giriş Yapılmayan': s.dist - s.checked, 'Giriş Oranı': s.dist > 0 ? ((s.checked / s.dist) * 100).toFixed(1) + '%' : '0%' }))), 'Grup_Bazlı');
...
function setReportViewS3(view) {
  ...
  if (cachedReportRows.length) {
    // Rapor zaten oluşturulmuşsa tekrar oluştur
    generateReportS3();
  }
}
...
function setupFilters() {
  if (filterProje) filterProje.addEventListener('input', applyFiltersToReport);
  if (filterDeğerlendirici) filterDeğerlendirici.addEventListener('input', applyFiltersToReport);
  if (filterGrup) filterGrup.addEventListener('input', applyFiltersToReport);
}
...
function initStep3() {
  ...
  if (!dropMain || !dropHistory || !dropChecked) {
    console.error('Step3 drop alanları bulunamadı!');
    return;
  }
  ...
  if (calculateBtnS3) {
    ...
    console.log('📊 Raporu Oluştur butonuna event bağlandı.');
  } else {
    console.error('calculateBtnS3 bulunamadı!');
  }
  ...
  // Sekmeler
  if (tabProje) tabProje.addEventListener('click', () => setReportViewS3('proje'));
  if (tabProjeKisi) tabProjeKisi.addEventListener('click', () => setReportViewS3('projekisi'));
  if (tabKisi) tabKisi.addEventListener('click', () => setReportViewS3('kisi'));
  if (tabGroup) tabGroup.addEventListener('click', () => setReportViewS3('group'));

  [tabProje, tabProjeKisi, tabKisi, tabGroup].forEach(b => {
